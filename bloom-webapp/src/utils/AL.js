...
// const host = 'http://192.168.199.243:3000/0.1/admin';
const host = 'https://flowerso2o.leanapp.cn/0.1/admin';
...
const avobject2withoutData = (avobject)=>{
...
const checkAVObjectChanges = (
  ...
  for (let key in params){
    ...
    // if (key==="introducePictures"){
    //   AL.log("introducePictures", avobject[key].every((obj)=>(isAVObject(obj))));
    // }
    ...
    else if (isDate(value) && isDate(avobject[key])){
    ...
    else if (isArray(value) && (isArray(avobject[key]) && avobject[key].every((obj)=>(!isAVObject(obj))))){
      if (JSON.stringify(value) !== JSON.stringify(avobject[key])){
          data[key] = value;
      }
    }
    else if (isArray(value) && (isArray(avobject[key]) && avobject[key].every((obj)=>(isAVObject(obj))))){

      if (value.length !== avobject[key].length){
          data[key] = value;
      }else if (value.some((obj)=>(avobject[key].map((avobj)=>(avobj.objectId)).indexOf(obj.objectId)===-1))){
          data[key] = value;
      }
    }
    else {
